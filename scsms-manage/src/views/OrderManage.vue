    ...
    const getOrderFun = (page: number, size: number): void => {
      ...
      getOrder().then((data: any) => {
        ...
        data.list.map((item: any) => {
          ...
          let foods = "";
          //const food = Array.from(item.food);
          for (let item1 of item?.food) {
            foods += " " + item1.count + "份" + item1.name;
          }
          orderItem.food = foods;
          console.log(orderItem.food);

          //orderItem.food = item.food;
          orderItem.createTime = formatTime(new Date(item.createdAt));
